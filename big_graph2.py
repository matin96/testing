...
def ml_prediction(node):
    ...
    # pr_df.to_csv("big graph2/pr_df.csv")
...
def rnn_prediction(node):
    ...
    test_size = 20
    train, val, test = dataset[:, :], dataset[train_size:, :], dataset[train_size:, :]
    window_size = 9
    ...
    # model1.save('big graph2/mymodel')
    # from keras.models import load_model
    #
    # model1 = load_model('big graph2/mymodel')
    model1.summary()
    ...
    train_results.to_csv("big graph2/train.csv")
    ...
    test_results.to_csv("big graph2/test.csv")
...
# # timeout = time.time() + 60  # 1 minutes from no
# # i = 0
# # while time.time() < timeout:
# #     # i = 0
# #     for li in edge_list:
# #         index = edge_list.index(li)
# #         x = li[i] + numpy.random.choice([0, 1, 2], p=[0.4, 0.4, 0.2]) * int(i/50) + 5 * int(numpy.random.normal(0, 3))
# #         li.append(x)
# #         edge_list[index] = li
# #     i += 1
# #     time.sleep(0.01)
...
#
#
# dataframe.to_csv("big graph2/big graph edges.csv")
# print("done!!!!!!")
dataframe = pd.read_csv("big graph2/big graph edges.csv", index_col=0)
dataframe.plot()
plt.show()
...
# edge_df.to_csv('big graph2/p_edge.csv')
# print("done1")

edge_df = pd.read_csv('big graph2/p_edge.csv', index_col=0)
graph_list2 = []
...
pickle.dump(graph_list2[-1], open('big graph2/graph_list2.pickle', 'wb'))
...
indeg = nx.in_degree_centrality(graph_list2[-1])
...
print("in deg centrality of last nodes: ", sorted(indeg.items(), key=lambda x:x[1], reverse=True))
...
#     num += 1
#     print("num=", num)
# node_df.to_csv("big graph2/p_node.csv")
# print("done2!")
node_df = pd.read_csv("big graph2/p_node.csv", index_col=0)
...
#
# phfs_df.to_csv("big graph2/phfs.csv")
# fs_df.to_csv("big graph2/function_scores.csv")

phfs_df = pd.read_csv("big graph2/phfs.csv", index_col=0)
fs_df = pd.read_csv("big graph2/function_scores.csv", index_col=0)
...
# workload_df['101'] = workload_df["API"]
workload_df.drop("API", inplace=True, axis=1)
# workload_df.drop("101", inplace=True, axis=1)

workload_df.to_csv("big graph2/workloads.csv")
predictions = []
# plt.suptitle("workloads and predictions:", fontsize=10)
...
#     if node == 101:
#         continue
#     pred, rms = ml_prediction(str(node))
#     # pred1 = pred[-1]
#     # predictions.append(pred1)
#     # predictions.append(ml_prediction(str(node))[len(ml_prediction(str(node))) - 2])
#     ridge_pred_df[node] = pred
#     rmss += rms
# ridge_pred_df.to_csv("big graph2/ridge_pred_df.csv")
ridge_pred_df = pd.read_csv("big graph2/ridge_pred_df.csv")
predictions = ridge_pred_df.iloc[-1, :]

workload_df = pd.read_csv("big graph2/workloads.csv", index_col=0)
...
UCPU_df.to_csv("big graph2/CPU util.csv")
Umem_df.to_csv("big graph2/memory util.csv")
...
predictions2 = pd.read_csv("big graph2/RNN.csv", index_col=0)
...
dm_df.to_csv("big graph2/predicted.csv")
...
# divided2 = []
# for item1, item2, item3 in zip(ucpu, conts, conts2):
#     if float(dm_df.columns[list(ucpu).index(item1)]) % 2 == 0:
#         item = item1 / item2
#         print("col ", dm_df.columns[list(ucpu).index(item1)], "item 1 = ", item1, " item 2 = ", item2)
#     else:
#         item = item1 / item3
#     divided2.append(item * 100)
#
# divided3 = []
# for item1, item2, item3 in zip(umem, conts, conts2):
#     if float(dm_df.columns[list(umem).index(item1)]) % 2 == 0:
#         item = item1 / item3
#     else:
#         item = item1 / item2
#     divided3.append(item * 100)

# dif_df.loc['w(t+1) - w(t)'] = wt1 - predictions2.iloc[-2, :]
dif_df.loc['w(t+1) - w(t)'] = wt1 - wt
dif_df.loc['ucpu(t)'] = divided2
dif_df.loc['umem(t)'] = divided3

dif_df = dif_df.round(4)
dif_df.to_csv("big graph2/differenced_dm.csv")
...
dif_df.to_csv("big graph2/normalized.csv")
...
#
# print("real node workload len:", len(real_node_workload.columns))
# real_node_workload.to_csv("big graph2/real node workloads.csv")

real_node_workload = pd.read_csv("big graph2/real node workloads.csv", index_col=0)
print("real node workload len:", len(real_node_workload.columns))

scaler1 = MinMaxScaler()
# workload_df_scaled = pd.DataFrame(columns=workload_df.columns)
workload_df_scaled = pd.DataFrame(scaler1.fit_transform(workload_df), columns=workload_df.columns)
# workload_df2 = pd.DataFrame(workload_df_scaled, columns=fs_df)
scaler2 = MinMaxScaler()
# real_node_workload_scaled = pd.DataFrame(columns=workload_df.columns)
real_node_workload_scaled = pd.DataFrame(scaler2.fit_transform(real_node_workload), columns=workload_df.columns)
# real_node_workload2 = pd.DataFrame(real_node_workload_scaled, columns=fs_df.columns)
...
for col in range(0, 100):
    ...
    MSE += ms
MSE /= 100
workload_mse_df.to_csv("big graph2/real_workload_mse.csv")
...
#
# rnn_df.to_csv("big graph2/rnn3.csv")
# ********** ***********
...
dif_df = pd.read_csv("big graph2/differenced_dm.csv", index_col=0)
...
def decision_making(timee, n_list):
    ...
    dm_df2.to_csv("big graph2/predicted2.csv")
    ...
    cpu_utilization_df.to_csv("big graph2/cpu_utilization.csv")
    mem_utilization_df.to_csv("big graph2/mem_utilization.csv")

    dif_df2 = dif_df2.round(4)

    dif_df2.to_csv(f'big graph2/dif_dfs/differenced_dm2T{timee}.csv')
    divisors = []
    ...
    dif_df.to_csv("big graph2/normalized2.csv")
    ...
    containers_df.to_csv("big graph2/containers.csv")
    return conts_pred
...
def decision_making2(timee, n_list):
    ...
    cpu_utilization_df2.to_csv("big graph2/cpu_utilization2.csv")
    mem_utilization_df2.to_csv("big graph2/mem_utilization2.csv")
    ...
    for col in dm_df2:
        ...
        if(cpu>90 or mem>90 or cpu<70 or mem<70):
            ...
            if (cpu > 90 or mem > 90):
                ...
                n -= 1
            # if int(col) % 2 == 0:
            #     max_cpu = last_cpu / 0.9
            #     n1 = int(max_cpu / 200) + 1
            #     max_mem = last_mem/0.9
            #     n2 = int(max_mem/25) + 1
            #     if(cpu>90 or mem>90):
            #         n = max(n1, n2) + 1
            #     elif(cpu<70 or mem<70):
    ...
    containers_df2.to_csv("big graph2/containers2.csv")
    return conts_pred
...
def decision_making3(timee, n_list):
    ...
    cpu_utilization_df2.loc[len(cpu_utilization_df2)] = divided2
    mem_utilization_df2.loc[len(mem_utilization_df2)] = divided3
    cpu_utilization_df2.to_csv("big graph2/cpu_utilization3.csv")
    mem_utilization_df2.to_csv("big graph2/mem_utilization3.csv")
    ...
    for col in dm_df2:
        ...
        # if(cpu2>90 or mem2>90 or cpu2<70 or mem2<70):
        #     n = containers_df2.iloc[-1, containers_df2.columns.get_loc(col)]
        if int(col) % 2 == 0:
            max_cpu = cpu / 0.9
            n1 = int(max_cpu / 200) + 1
    ...
    containers_df3.to_csv("big graph2/containers3.csv")
...
memory_util = pd.read_csv("big graph2/mem_utilization.csv", index_col=0)
...
memory_util2 = pd.read_csv("big graph2/mem_utilization2.csv", index_col=0)
...
cpu_util = pd.read_csv("big graph2/cpu_utilization.csv", index_col=0)
...
cpu_util2 = pd.read_csv("big graph2/cpu_utilization2.csv", index_col=0)
...
no_usage_cpu = pd.read_csv("big graph2/cpu_utilization.csv", index_col=0)
...
no_usage_cpu.to_csv("big graph2/ no_CPU.csv")

no_usage_cpu2 = pd.read_csv("big graph2/cpu_utilization2.csv", index_col=0)
...
no_usage_cpu2.to_csv("big graph2/no_CPU2.csv")
...
no_usage_mem = pd.read_csv("big graph2/mem_utilization.csv", index_col=0)
...
no_usage_mem.to_csv("big graph2/no_mem.csv")

no_usage_mem2 = pd.read_csv("big graph2/mem_utilization2.csv", index_col=0)
...
no_usage_mem2.to_csv("big graph2/no_mem2.csv")
...
containers_df = pd.read_csv("big graph2/containers.csv", index_col=0)
